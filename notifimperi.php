<?php
   ...
   //***********************************************************************************************************************
   // V1.1 : Notification TTS via API ImperiHome. Compatible Chatbot et Ask
	...
	$notif = "";
	if ($value != "" && $value >= 0 && $key != "plugin.parameters.imperihomeIP" && $event != "plugin.parameters.imperihomePort") {
		...
		$url = "http://".$ip.":".$port."/api/rest/speech/tts?times=1&vol=100&text=".$value1;
	...
	function sdk_noaccent($text) {
		...
		$utf8_values = array('a', 'A', 'I',	'i', 'ai', 'ai', 'o',	'O', 'u', 'U', 'ss', 'ss', 'n', 'N');
  ...
  $out = str_replace('%B0','degré%20',$out);
  return $out;
	} 
?>
